...
ALTER  PROCEDURE dbo.procBuildBatesDocPage
(
...
-- check that we are using a bates set that has a production origin
IF NOT EXISTS( Select * from tblBatesSet
        WHERE BatesSetID = @pi_BatesSetID AND BatesOriginCd = 'PRD'
)
BEGIN
    SELECT @ErrorMessage = 'Error in ' + @spName + ': Bates set does not exist or did not originate from a production set.'
    RAISERROR(@ErrorMessage, 16, 1) WITH LOG, SETERROR
    RETURN(@@Error)
END
...
IF EXISTS( Select * from #Pages p
        JOIN tblOutputPageBates opb (NOLOCK) on p.ItemIdentityID = opb.ItemIdentityID
...
--  Order by BatesPrefixID,dbo.fnGetBatesSort(ItemIdentityID)
...
from #PrefixStartDoc psd
    Left Outer JOIN tblOutputPageBates opb (NOLOCK)
...
BEGIN TRAN
    ...
    from #Pages p
    where not exists(
        Select * from tblOutputPageBates opb (NOLOCK)
