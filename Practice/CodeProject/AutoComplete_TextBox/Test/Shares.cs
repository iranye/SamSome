...
namespace Test
{
     ...
     * 
     * 
     * File Source: http://www.codeproject.com/KB/IP/networkshares.aspx
     * 
     ***/
    ...
    public class Share
    {
        ...
        public bool IsFileSystem
        {
            ...
            {
                // Shared device
                if (0 != (this.mShareType & ShareType.Device)) return false;

                // IPC share
                if (0 != (this.mShareType & ShareType.IPC)) return false;

                // Shared printer
                if (0 != (this.mShareType & ShareType.Printer)) return false;

                // Standard disk share
                if (0 == (this.mShareType & ShareType.Special)) return true;

                // Special disk share (e.g. C$)
                if (ShareType.Special == this.mShareType && null != this.mNetName && 0 != this.mNetName.Length)
                    return true;
                else
                    return false;
            }
        ...
        public DirectoryInfo Root
        {
            get
            {
                ...
                {
                    if (null == this.mSserver || 0 == this.mSserver.Length)
                        if (null == this.mPath || 0 == this.mPath.Length)
                            return new DirectoryInfo(this.ToString());
                        else
                            return new DirectoryInfo(this.mPath);
                    else
                        return new DirectoryInfo(this.ToString());
                }
                else
        ...
        public override string ToString()
        {
            if (null == this.mSserver || 0 == this.mSserver.Length)
            {
                return string.Format(@"\\{0}\{1}", Environment.MachineName, this.mNetName);
            }
            else
                return string.Format(@"\\{0}\{1}", this.mSserver, this.mNetName);
        }
        ...
        {
            if (!this.IsFileSystem) return false;
            if (null == path || 0 == path.Length) return true;

            return path.ToLower().StartsWith(this.mPath.ToLower());
        }
    }
    #endregion
    ...
    public class ShareCollection : ReadOnlyCollectionBase
    {
        ...
        private static ShareCollection mLocal = null;
        ...
        {
            this.mServer = string.Empty;
            EnumerateShares(this.mServer, this);
        }
        ...
        protected static void EnumerateShares9x(string server, ShareCollection shares)
        {
            ...
            ushort cbBuffer = (ushort)(MaxSI50Entries * size);

            IntPtr pBuffer = Marshal.AllocHGlobal(cbBuffer); // On Win9x, must allocate buffer before calling API
        ...
        {
            InnerList.Add(new Share(this.mServer, netName, path, shareType, remark));
        }
        ...
        protected struct SHARE_INFO_50
        {
            ...
            public ShareType ShareType
            {
                get { return (ShareType)((int)this.BShareType & 0x7F); }
            }
        }
        ...
        protected struct SHARE_INFO_1_9x
        {
            ...
            public ShareType ShareType
            {
                get { return (ShareType)((int)this.BShareType & 0x7FFF); }
            }
        }
        #endregion
    }
